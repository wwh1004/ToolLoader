...
#nullable disable
	...
	static HashSet<string> resolvingAssemblies;
#nullable restore
	...
	public static void Execute(string[] args) {
		...
		var toolPath = Path.GetFullPath(args[0]);
		dependencyResolver = new AssemblyDependencyResolver(toolPath);
		resolvingAssemblies = new HashSet<string>();
	...
	static Assembly? ResolveAssembly(AssemblyLoadContext loadContext, AssemblyName assemblyName) {
		try {
			if (!resolvingAssemblies.Add(assemblyName.FullName))
				return null;
			var assembly = loadContext.LoadFromAssemblyName(assemblyName);
			resolvingAssemblies.Remove(assemblyName.FullName);
			return assembly;
		}
		catch {
	...
	static IntPtr ResolveUnmanagedDll(Assembly assembly, string unmanagedDllName) {
		var unmanagedDllPath = dependencyResolver.ResolveUnmanagedDllToPath(unmanagedDllName);
